...
- ~~Replace subprocess (`Process()`) with `ContainerAPIClient` SDK~~ ✓ Done — removed the `container` CLI subprocess invocation and the `--container-path` CLI option. `ContainerExecutor` now drives `container-apiserver` directly via XPC using `ContainerClient`, `ClientImage`, `ClientKernel`, and `ClientProcess` from the `apple/container` v0.10.0 package (`ContainerAPIClient` + `ContainerResource` products). This eliminates the launched-process dependency and makes the executor a first-class SDK consumer.
- ~~Fix memory stats collection to poll concurrently~~ ✓ Done — `client.stats(id:)` was previously called once after `process.wait()`, reliably returning zero because cgroup data is torn down at exit. Now polled in a `Task.detached` loop every 250 ms while the container is running; peak `memoryUsageBytes` from `ContainerStats` is tracked and cancelled immediately after `wait()` returns.
- ~~Extract `ContainerBackend` and `ContainerProcess` protocols for dependency injection~~ ✓ Done — `ContainerBackend.swift` holds the two protocols; `LiveContainerBackend.swift` holds the production actor (`LiveContainerBackend`) and the `ClientProcessBridge` adapter. `ContainerExecutor` takes `any ContainerBackend` via its initialiser, defaulting to `LiveContainerBackend()`. Same split applied to `RemoteExecutor`: `RemoteREAPIBackend.swift` holds the protocol; `LiveRemoteREAPIBackend.swift` holds the production `final class` that owns the gRPC connection lifecycle (replacing the inline `deinit` on the executor). `RemoteExecutor` now has a test-only `init(localCAS:remote:)` that skips network setup.
...
- ~~Add `ContainerExecutorTests` using `MockContainerBackend`~~ ✓ Done — 10 tests: cache hit (no backend calls), success caches result, failure not cached + `[HERMETIC]` tag, OOM `[FLAKY]` tag, `skipCacheLookup` bypass, `resolveImage` call count, stdout capture, container deleted on success, container deleted on failure, output file collection.
- ~~Add `RemoteExecutorTests` using `MockRemoteREAPIBackend`~~ ✓ Done — 14 tests: `collectDigests` tree walking (flat + recursive + no-duplicate), `extractResult` parsing (success, error, empty stream, not-done skipped), mock-backend integration (no upload when nothing missing, missing blobs fetched + uploaded, `skipCacheLookup` forwarded, remote result returned). `collectDigests(for:cas:)` and `extractResult(from:)` are exposed as `internal static` methods on `RemoteExecutor` to enable direct unit testing.
- ~~Add `mise run test:coverage` task and configure coverage exclusions~~ ✓ Done — runs `swift test --enable-code-coverage` and reports via `xcrun llvm-cov report`. `Live*.swift` files (daemon/network glue requiring a live XPC or gRPC endpoint) are excluded from the report via `-ignore-filename-regex '.*Live[A-Z].*'`. Protocol-only files (`ContainerBackend.swift`, `RemoteREAPIBackend.swift`) contain no executable lines and are omitted from totals automatically. Total: **88 tests in 16 suites**.
